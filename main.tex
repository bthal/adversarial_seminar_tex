...
This success, however, is built upon a surprisingly fragile foundation. In 2013, Szegedy et al.\ \cite{szegedy2013intriguing} showed that state-of-the-art classifiers can be fooled by \textit{adversarial examples}: inputs altered by a perturbation so small that it is imperceptible to a human, yet enough to make the model produce a confident but incorrect prediction. Such examples are neither rare accidents nor isolated mistakes of a single model: the same perturbed input frequently fools different architectures trained on different data, furthermore the vulnerability appears across all kinds of machine learners~\cite{goodfellow2014explaining, biggio2018wild}.

This report serves as a broad introduction to the topic of adversarial attacks. In our examination of specific attack methods, we thus concentrate on the classical setting of evasion and poisoning attacks against image classifiers, surveying the ideas and methods that have shaped the research area.
...
given a data point \(x\), a limit \(\delta\), and a loss function \(L(\theta, x, y)\), to find the 'worst' adversarial example (maximises loss) in the $L_\infty$-ball around $x$
...
Different variants of FGSM were proposed by Kurakin et al.~\cite{kurakin2017physical,kurakin2017scale}.
...
To find adversarial examples of a specific class \(y_{target}\) instead of maximising the loss function, FGSM can be modified to maximise that classes probability \(p(y_{target} | X)\). This can be achieved with a single step in direction of \(sign(\nabla_X log(p(y_{target} | X)))\) \cite{kurakin2017physical}
...
However, this strength has a caveat. As we will see in Section~\ref{sssec:momentum}, not every adversarial example corresponding to a local minimum is equally transferable to other models.
...
where $r_i$ is the smallest perturbation that crosses the linearised boundary. The procedure is repeated until the classifier changes its prediction.
...
where $\|\delta\|_p$ measures the size of the perturbation and $f$ is a logit-based loss function that operates on the network's raw logits rather than the softmax output probabilities. The loss is defined as:
...
In a transfer attack, an adversary first constructs a substitute model that he has full access to as a stand-in for a target black-box model. On this substitute, white-box attacks like the ones described above can be performed to find adversarial examples. Since they generally transfer from one model to another, these can then be used to attack the actual target model.
...
\]
where the first term compares the target t and poison candidates x in feature space (the more similar, the better), and the second penalises difference from the base instance. The general idea is to find a poison instance close to b, therefore likely to be labeled the same, yet close to t in feature space (output of second-to-last layer), so t is treated the same as b by the model. %Shafahi et al. argue that a suitable x can be found due to f being supposedly highly nonlinear. 
...
The attack itself is similar to Shafahi et al.'s. The adversary chooses a source and a target image from the data distribution. He then applies his trigger to the source, obtaining the patched source. Like before, the poison sample is optimised to be close to the target in pixel space (so it probably receives the target label) while close to the patched source in feature space (so the model has difficulties distinguishing them). Note that the roles of source and target are flipped in the notation compared to Shafahi et al.

Let \(\tilde{s}\) be the patched source, \(t\) the target, and \(\epsilon\) a small, chosen threshold. Saha et al. optimise
\[
...
To optimise, they use projected gradient descent (Section~\ref{PGD}). However, in each optimisation step, a new randomly-patched source is selected so the backdoor generalises to arbitrary images and patch placements. %...% !!! ... multiple at once, 1:1 problem
...
Their approach revolves around optimising the trigger pattern used under a constraint so it is harder to notice for humans (as in evasion attacks). 
...
\begin{figure}[t]
	\centering
	\subcaptionbox{A 3D-printed turtle that is consistently misclassified as a rifle. Reproduced from \cite{athalye2018synthesizing}\label{fig:adversarialturtle}}{\includegraphics[height=2.7cm]{adversarial_turtle}}\hfill
	...
	\subcaptionbox{A perturbed logo to be recognised as a \textit{No overtaking} sign. Reproduced from \cite{sitawarin2018darts}\label{fig:adversariallogo}}{\includegraphics[height=2.7cm]{adversarial_logo}}
