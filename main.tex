...
Over the past decade, deep neural networks have become the dominant approach to a broad range of perception tasks from image classification and speech recognition to natural language understanding in many cases matching or surpassing human performance on standard benchmarks.\cite{lecun2015deep, he2015delving} Due to the strength of these results, such models are increasingly deployed in critical, safety-relevant settings, such as autonomous driving\cite{grigorescu2020survey}  or biometric identification\cite{schroff2015facenet}.

This success, however, rests on a surprisingly fragile foundation. In 2013, Szegedy et al.\ \cite{szegedy2013intriguing} showed that state-of-the-art classifiers can be fooled by \textit{adversarial examples}: inputs altered by a perturbation so small that it is imperceptible to a human, yet enough to make the model produce a confident but incorrect prediction. One example to show the effect in action can be seen in Figure~\ref{fig:panda_gibbon}. Such examples are neither rare accidents nor quirks of a single model: the same perturbed input frequently fools different architectures trained on different data, and the vulnerability appears across all kinds of machine learners.\cite{goodfellow2014explaining, biggio2018wild}

This report is intended as a broad introduction to the topic of adversarial attacks. In our examination of specific attack methods, we thus concentrate on the classical setting of evasion and poisoning attacks against image classifiers, surveying the ideas and methods that have shaped the research area.
...
\textit{Adversarial Attacks} can be described as inputting {adversarial examples} into a machine learning system.\cite{brendel2017decision} These two terms where introduced by \cite{szegedy2013intriguing} and have since been used in different variations, however we here use the following definition:
...
\begin{figure}[t]
    ...
    \begin{subfigure}[t]{0.49\textwidth}
        \centering
        \includegraphics[height=3.5cm]{fig_linearity.pdf}
        \caption{The linear explanation: under an $L_\infty$ budget the per-pixel change $\lVert\eta\rVert_\infty=\epsilon$ stays fixed, while the output change $w^\top\eta=\epsilon\,m\,n$ grows linearly with the dimension $n$ until it crosses the threshold that flips the prediction.}
        \label{fig:linearity}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.49\textwidth}
